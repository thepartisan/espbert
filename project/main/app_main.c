...
static void mqtt_send(int val)
{
    char buf[128];
    memset(buf, 0, sizeof(buf));
    sprintf(&buf[3], "{\"%s\":%d}", "adc1: ", val);
    uint16_t len = strlen(&buf[3]);
    buf[0] = 0x05 /*data_type_simple_json_without_time*/;
    buf[1] = len >> 8;
    buf[2] = len & 0xFF;
    esp_mqtt_client_publish(client, "/topic/adc1", buf, len + 3, 0, 0);
}

static void dac_task(void)
{
...
static esp_err_t mqtt_event_handler(esp_mqtt_event_handle_t event)
{
    ...
    switch (event->event_id) {
        case MQTT_EVENT_CONNECTED:
            ...
            msg_id = esp_mqtt_client_subscribe(client, "/topic/temp", 0);
            ESP_LOGI(TAG, "sent subscribe successful, msg_id=%d", msg_id);
